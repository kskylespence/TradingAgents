...
- **The Docker image installs exact, hash-checked dependencies.** Every
  ...
  and a hijacked release would have shipped on the next build.
  `requirements.lock` (generated by `uv pip compile --generate-hashes`,
  see `docs/RELEASING.md` §2.3) pins all 110 packages, the Dockerfile
  installs it with `pip install --require-hashes`, and the local packages
  build `--no-build-isolation` with the locked setuptools, so nothing is
  fetched unpinned. `pip check` runs at the end of the install.
- **Dependency floors raised past known advisories.** Twelve direct
  dependencies allowed versions with published advisories (e.g.
  `python-multipart>=0.0.20` — fixes up to 0.0.31; `cryptography>=44` —
  seven, fixed in 50.0.0; `langgraph-checkpoint-sqlite>=2.0.0` — four).
  Each floor is now the lowest release `pip-audit` reports clean, and
  `starlette` — imported directly in eight places — gets its own floor
  instead of whatever fastapi allows. Anyone installing without the lock
  (CLI users, dev setups) can no longer resolve to a known-vulnerable
  release.
- **pytest 9 (CVE-2025-71176) and pytest-asyncio 1.x.** pytest through
  9.0.2 used predictable `/tmp/pytest-of-<user>` directories, attackable
...
- **Build stage on Node 24 LTS; no compiler in the runtime image.** Node 20
  ...
  every locked package has a prebuilt wheel.
