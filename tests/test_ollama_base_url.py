...
# NOTE: The previous tests `test_ollama_model_labels_no_local_suffix` and
# `test_ollama_offers_custom_model_id` were removed when the Ollama branch
# of the static MODEL_OPTIONS dict was deleted. Ollama models are now
# discovered at runtime via `app.services.ollama_models.list_ollama_models`,
# which calls `GET {OLLAMA_BASE_URL}/models`. The "Custom model ID"
# affordance is intentionally NOT offered for Ollama anymore — the
# discoverable model list is canonical, so picking anything outside it
# would just reproduce the 404s that motivated this change.
