...
| `ADMIN_PASSWORD_HASH` | bcrypt hash of the admin password (`$2b$12$…`). The plaintext password is never stored. Required (no default); enforced `min_length=60` because every bcrypt hash is exactly 60 chars. Generate with the recipe in [`DEPLOY.md` Step 1](../../DEPLOY.md). |
